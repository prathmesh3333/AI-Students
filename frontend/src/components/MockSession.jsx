...
const MockSession = () => {
  ...
  const flushTTSQueue = () => {
    ...
    if (ttsQueueRef.current.length === 0) {
      if (streamingDoneRef.current) {
        setIsSpeaking(false);
        // Only start listening if not already listening (skip button starts it at 300ms)
  ...
  const stopSpeaking = () => {
    ...
    ttsRawBufferRef.current = "";
    ttsSpeakingRef.current = false;
    // Mark streaming done so flushTTSQueue's onend won't try to start recognition again
    streamingDoneRef.current = true;
    setIsSpeaking(false);
    setLoading(false);
    setTranscript("");
    ...
    setInterimTranscript("");
    shouldAutoSendRef.current = false;
    // Start mic after a short delay (safeStartListening guards against double-start)

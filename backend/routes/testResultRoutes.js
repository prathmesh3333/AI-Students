...
const ViolationSnapshot = require("../models/ViolationSnapshot");
...
// Save a violation snapshot
router.post("/violation-snapshot", async (req, res) => {
  try {
    const { testId, rollNo, studentName, violationType, imageData } = req.body;

    if (!testId || !rollNo || !studentName || !violationType || !imageData) {
      return res.status(400).json({ success: false, error: "Missing required fields" });
    }

    const snapshot = new ViolationSnapshot({ testId, rollNo, studentName, violationType, imageData });
    await snapshot.save();

    res.json({ success: true });
  } catch (error) {
    console.error("Error saving violation snapshot:", error);
    res.status(500).json({ success: false, error: "Failed to save snapshot" });
  }
});

// Get all violation snapshots for a test (teacher/admin view)
router.get("/violation-snapshots/:testId", async (req, res) => {
  try {
    const snapshots = await ViolationSnapshot.find({ testId: req.params.testId })
      .sort({ capturedAt: -1 });
...
// Get violation snapshots for a specific student in a test
router.get("/violation-snapshots/:testId/:rollNo", async (req, res) => {
  try {
    const { testId, rollNo } = req.params;
    const snapshots = await ViolationSnapshot.find({ testId, rollNo }).sort({ capturedAt: 1 });
    res.json({ success: true, snapshots });
  } catch (error) {
